"""Wallet: external holdings, paper balance, and portfolio history.

Moved out of `endpoints/wallet.py`, which was 692 lines of third-party
integrations — Coinbase, Ethplorer, an Ethereum JSON-RPC node, the Solana RPC —
living inside a routing module. Routing modules should say what a URL maps to;
they should not also be where the Ethplorer response shape is parsed.

Every external call here is best-effort and returns a `status` field. A wallet
provider being down must degrade one section of the response rather than failing
the request: the paper balance and the realised PnL come from the local database
and are still worth serving when Ethplorer is not answering.
"""
...
# schema, and this is the one remaining shared constant.
