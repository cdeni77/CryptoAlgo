...
BOOK_BUDGET=90000 BOOK_WINDOWS=25000 python -u _collect_book.py >> "$LOG/book.log" 2>&1
say "         $(wc -l < data/book_full.jsonl 2>/dev/null || echo 0) windows"
depth

say "JOB 3/3  Polymarket — discover to 2026-03-02, then the full book"
PM_STAGE=both PM_PAGES=4000 PM_PRICED=25000 python -u _collect_pm.py >> "$LOG/pm.log" 2>&1
