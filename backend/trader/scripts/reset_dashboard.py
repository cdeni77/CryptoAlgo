...
`--rebase` additionally sets `starting_bankroll` to the CURRENT bankroll, so the
new equity curve starts flat at 0% instead of carrying the old return forward.
That does change position sizing, because `decide()` sizes off
`config.starting_bankroll` when not compounding -- which is why it is a separate
flag and is printed before it happens.
"""
...
def main(argv=None) -> int:
    ...
    with writer._session() as session:                    # noqa: SLF001
        ...
        if args.rebase:
            logger.info('rebasing starting_bankroll %.2f -> %.2f '
                        '(this changes position sizing)',
